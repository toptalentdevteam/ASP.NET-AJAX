...
{
    public class ProfileWorkerService : Controller
    {
        [HttpGet]
        public ProfileWorkerModel GetProfileWorker(string UserUniqueId)
        {
            var client = new RestClient("https://apiv1.evirtualassistants.com/hire/worker-profile/" + UserUniqueId);
            var request = new RestRequest(Method.GET);
            request.AddHeader("postman-token", "732c0cf1-f554-c883-9ffa-c47e5360c944");
            ...
            request.AddHeader("accept-encoding", "gzip, deflate, utf8");
            request.AddHeader("accept", "application/json, text/plain, */*");
            var response = client.Execute(request);
            string jsonString = response.Content;
            JavaScriptSerializer js = new JavaScriptSerializer();
            ProfileWorkerModel detailsWorkers = js.Deserialize<ProfileWorkerModel>(jsonString);
