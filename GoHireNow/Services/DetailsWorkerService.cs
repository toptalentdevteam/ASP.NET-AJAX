using System.Diagnostics;
using Microsoft.AspNetCore.Mvc;
using Microsoft.Extensions.Logging;
using GoHireNow.Models;
using RestSharp;
using Newtonsoft.Json.Linq;
using Newtonsoft.Json;
using System.Net.Http;
using System.Text;
using Brotli;
using System.IO;
using Nancy.Json;
using System.Collections.Generic;


namespace GoHireNow.Controllers
{
    public class DetailsWorkerService : Controller
    {
        [HttpGet]
        public DetailsWorkerModel GetDetailsWorker(string id, string pageIndex)
        {
            if (string.IsNullOrEmpty(pageIndex))
            {
                pageIndex = "1";
            }
            if (id == "all")
            {
                id = "0";
            }

            var client = new RestClient("https://apiv1.evirtualassistants.com/hire/workers/" + id + "?size=10&page=" + pageIndex);
            var request = new RestRequest(Method.GET);
            request.AddHeader("postman-token", "2dd7b563-7b35-f53b-c0ff-d93577ec5af7");
            request.AddHeader("cache-control", "no-cache");
            request.AddHeader("user-agent", "Mozilla/5.0 (Linux; Android 6.0; Nexus 5 Build/MRA58N) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/89.0.4389.90 Mobile Safari/537.36");
            request.AddHeader("sec-fetch-site", "same-site");
            request.AddHeader("sec-fetch-mode", "cors");
            request.AddHeader("sec-fetch-dest", "empty");
            request.AddHeader("sec-ch-ua-mobile", "?1");
            request.AddHeader("sec-ch-ua", "\"Google Chrome\";v=\"89\", \"Chromium\";v=\"89\", \";Not\"A Brand\";v=\"99\"");
            request.AddHeader("referer", "https://www.evirtualassistants.com/");
            request.AddHeader("origin", "https://www.evirtualassistants.com");
            request.AddHeader("authorization", "Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..czN8zO-gfoWouo_sYUaAjDZtUxrxdi2q0sQ1Cwc_Yko");
            request.AddHeader("accept-language", "vi-VN,vi;q=0.9,fr-FR;q=0.8,fr;q=0.7,en-US;q=0.6,en;q=0.5");
            request.AddHeader("accept-encoding", "gzip, deflate, utf-8");
            request.AddHeader("accept", "application/json, text/plain, */*");

            var response = client.Execute(request);
            string jsonString = response.Content;
            JavaScriptSerializer js = new JavaScriptSerializer();
            DetailsWorkerModel detailsWorkers = js.Deserialize<DetailsWorkerModel>(jsonString);
            return detailsWorkers;
        }
    }
    public class RelatedWorkersService : Controller
    {
        [HttpGet]
        public RelatedWorkersModel GetRelatedWorkers(string id, string pageIndex)
        {
            if (string.IsNullOrEmpty(pageIndex))
            {
                pageIndex = "1";
            }
            if (id == "all")
            {
                id = "0";
            }
            var client = new RestClient("https://apiv1.evirtualassistants.com/hire/relatedworkers/" + id + "?page=1&size=" + pageIndex);
            var request = new RestRequest(Method.GET);
            request.AddHeader("postman-token", "2dd7b563-7b35-f53b-c0ff-d93577ec5af7");
            request.AddHeader("cache-control", "no-cache");
            request.AddHeader("user-agent", "Mozilla/5.0 (Linux; Android 6.0; Nexus 5 Build/MRA58N) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/89.0.4389.90 Mobile Safari/537.36");
            request.AddHeader("sec-fetch-site", "same-site");
            request.AddHeader("sec-fetch-mode", "cors");
            request.AddHeader("sec-fetch-dest", "empty");
            request.AddHeader("sec-ch-ua-mobile", "?1");
            request.AddHeader("sec-ch-ua", "\"Google Chrome\";v=\"89\", \"Chromium\";v=\"89\", \";Not\"A Brand\";v=\"99\"");
            request.AddHeader("referer", "https://www.evirtualassistants.com/");
            request.AddHeader("origin", "https://www.evirtualassistants.com");
            request.AddHeader("authorization", "Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..czN8zO-gfoWouo_sYUaAjDZtUxrxdi2q0sQ1Cwc_Yko");
            request.AddHeader("accept-language", "vi-VN,vi;q=0.9,fr-FR;q=0.8,fr;q=0.7,en-US;q=0.6,en;q=0.5");
            request.AddHeader("accept-encoding", "gzip, deflate, utf-8");
            request.AddHeader("accept", "application/json, text/plain, */*");

            var response = client.Execute(request);
            string jsonString = response.Content;

            string temp = "{\"workers\":" + jsonString + "}";

            JavaScriptSerializer js = new JavaScriptSerializer();
            RelatedWorkersModel relatedWorkers = js.Deserialize<RelatedWorkersModel>(temp);
            return relatedWorkers;
        }
    }
    public class SkillsService : Controller
    {
        [HttpGet]
        public string GetSkills()
        {
            var client = new RestClient("https://devapiv1.evirtualassistants.com/lookup/skills");
            var request = new RestRequest(Method.GET);
            request.AddHeader("postman-token", "2dd7b563-7b35-f53b-c0ff-d93577ec5af7");
            request.AddHeader("cache-control", "no-cache");
            request.AddHeader("user-agent", "Mozilla/5.0 (Linux; Android 6.0; Nexus 5 Build/MRA58N) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/89.0.4389.90 Mobile Safari/537.36");
            request.AddHeader("sec-fetch-site", "same-site");
            request.AddHeader("sec-fetch-mode", "cors");
            request.AddHeader("sec-fetch-dest", "empty");
            request.AddHeader("sec-ch-ua-mobile", "?1");
            request.AddHeader("sec-ch-ua", "\"Google Chrome\";v=\"89\", \"Chromium\";v=\"89\", \";Not\"A Brand\";v=\"99\"");
            request.AddHeader("referer", "https://www.evirtualassistants.com/");
            request.AddHeader("origin", "https://www.evirtualassistants.com");
            request.AddHeader("authorization", "Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..czN8zO-gfoWouo_sYUaAjDZtUxrxdi2q0sQ1Cwc_Yko");
            request.AddHeader("accept-language", "vi-VN,vi;q=0.9,fr-FR;q=0.8,fr;q=0.7,en-US;q=0.6,en;q=0.5");
            request.AddHeader("accept-encoding", "gzip, deflate, utf-8");
            request.AddHeader("accept", "application/json, text/plain, */*");

            var response = client.Execute(request);
            //Debug.WriteLine("response.Content---->" + response.Content);
            return response.Content;
        }
    }
}
